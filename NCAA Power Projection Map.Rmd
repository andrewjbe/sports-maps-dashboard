...
for (i in seq_len(nrow(counties))) {
  closest_[[i]] <- pnts_sf[which.max(
    (ds_top$adj_em) * (1000 / sf::st_distance(pnts_sf, counties[i, ]))
    ), ]
...
> Each county is "controlled" by the school with the highest "power projection" score relative to that county. This score is equal to schools' Kenpom Adj. Efficiency Margin multiplied by the distance between the school and the nearest county border, in kilometers. In other words, each county is controlled by the strongest nearby team, with stronger teams having a longer "reach" (to keep things from being too crowded, only the 30 best teams in the county by Adj. EM can own land). Data source: [Kenpom.com](https://kenpom.com/)
